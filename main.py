...
def create_window():
    ...
    cmds.button(label='Create Controllers', command=lambda x: hand.ctrlHand(),width=100)
    cmds.setParent('..')
    cmds.rowLayout(numberOfColumns=3, columnWidth3=[75,75,150])
    cb_jnt_foot = cmds.checkBox(label="Joints")
    cb_ctrl_foot = cmds.checkBox(label="Ctrl")
    cmds.button(label='Mirror', command=lambda x:hand.mirorHand(cb_jnt_foot,cb_ctrl_foot),width=100)
    cmds.setParent('..')
    cmds.separator(h=8)
    cmds.setParent('..')
    ...
    cmds.button(label='Match Auto', command=lambda x:tools.matchIkFk(2),width=150)
    cmds.button(label='Fk/Ik', command=lambda x:tools.matchIkFk(0),width=75)
    cmds.button(label='Ik/Fk', command=lambda x:tools.matchIkFk(1),width=75)
    # Show the window
