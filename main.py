...
def create_window():
    ...
    cmds.separator(h=2)
    cmds.rowLayout(numberOfColumns=2, columnWidth2=[35,265])
    cb_notTheFirst=cmds.checkBox(label="- 1",v=False)
    cmds.button(label='Select Bind', command=lambda x:tools.selectJnt("bind",True,cb_notTheFirst),w=265)
    cmds.setParent('..')
    cmds.separator(h=5)
    
    cmds.rowLayout(numberOfColumns=3, columnWidth3=[100,100,100])
    cnNameFind = cmds.textField(placeholderText="Find",text="drv",w=100)
    cmds.button(label='Select Jnt', command=lambda x:tools.selectJnt(cnNameFind,True,cb_notTheFirst),width=100)
    cmds.button(label='Select All', command=lambda x:tools.selectJnt(cnNameFind,False,cb_notTheFirst),width=100)
    cmds.setParent('..')  
    cmds.frameLayout(label='Constraints',w = 300, collapsable=True, collapse=True)
    cmds.rowLayout(numberOfColumns=6, columnWidth6=[50,50,50,50,50,60])
    ...
    cmds.text(label="_________")
    cmds.separator(h=4)
    cmds.frameLayout(label='Aim On Curve Jnts', collapsable=True, collapse=True,w = 300)
    cmds.text(label=" - Select Curve (the one with joint on it) ", w = 300, align = "left")
    cmds.text(label=" - Select Thejoints on it ", w = 300, align = "left")
    cmds.rowLayout(numberOfColumns=3, columnWidth3=[100,100,100])
    txt_nom_Centre = cmds.textField(placeholderText="Obj Nom")
    int_path = cmds.intField(value=1)
    cmds.setParent('..')
    cmds.rowLayout(numberOfColumns=2, columnWidth2=[150,150])
    cbObjUp_Aim=cmds.textField(placeholderText="CTRL UP",w=150,enable=False)
    enum_dropdown_Aim = cmds.optionMenu(label="World Up Type", changeCommand=lambda *args:update_text_field(enum_dropdown_Aim,cbObjUp_Aim,*args))
    cmds.menuItem(label="Scene Up")
    cmds.menuItem(label="Object Up")
    cmds.menuItem(label="Object Rotation Up")
    cmds.menuItem(label="Vector")
    cmds.menuItem(label="Normal")
    cmds.setParent('..')    
    cmds.button(label='Create Eye Shit', command=lambda x:tools.aimOnCurveAdapt(txt_nom_Centre,int_path,enum_dropdown_Aim,cbObjUp_Aim),width=150)
    cmds.setParent('..')
    cmds.setParent('..')
