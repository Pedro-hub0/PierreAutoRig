...
def create_window():
    ...
    # Create the window
    window_name = cmds.window("myWindow", title="MatchIkFk_PierreAuto", widthHeight=(410, 180), sizeable=False)
    ...
    cmds.rowLayout(numberOfColumns=4, columnWidth4=[100,100,100,100])
    cmds.button(label='Rivets Up', command=lambda x:modules.RivetGael.gael_Rivet("Riv_Up_01"))
    cmds.button(label='Rivets Dwn', command=lambda x:modules.RivetGael.gael_Rivet("Riv_Dwn_01"))
    cmds.button(label='Jnt', command=lambda x:createJnt())
    cmds.button(label='Blend', command=lambda x:createBlendRivet(speed,number))
    cmds.setParent('..')
    cmds.setParent('..')

    cmds.frameLayout(label='Curve VERSION',w = 300, collapsable=True, collapse=True)
    cmds.button(label='Blend', command=lambda x:createBlendCurve(speed,number))
    cmds.setParent('..')
    # RENAME
...
def createBlendCurve(speed,l):
    ...
    for i in range(1,length+1):
        ...
        print(f'{i}   {pocUp[i-1]}   {pocDwn[i-1]} ')
